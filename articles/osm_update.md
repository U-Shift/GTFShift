...
requiring credentials to be run on either of them.

This script considers credentials to be stored at environment variables
(`OSM_USER` and `OSM_PASSWORD`), to avoid hardcoding them in the script.
When running it, either replace them manually by your credentials or,
better, set them as environment variables, using
`usethis::edit_r_environ()` on R, or by exporting them in the terminal.
