...
## GTFShift OSM contributions

GTFShift development team regularly uses this script to contribute to
OSM, updating GTFS shapes and routes data for different cities. Refer to
[the changesets
page](https://openstreetmap.org/user/Gon%C3%A7alo%20Matos/history) for a
complete list on the changes applied and to the [GitHub
releases](https://github.com/U-Shift/GTFShift/releases) for the detailed
logs and geometries.
...
run_date = "20260505"
base_folder = "osm_match" 

df = pd.read_csv(f"{base_folder}/shapes_match_{area}_gtfs{gtfs_date}_run{run_date}.csv") # CSV with columns osm_id, shape_id and route_id (optional)
len(df)
...
assert osmapi.__version__ >= "5.0.0", "Please update osmapi to version 5.0.0 or higher to ensure compatibility"
...
``` python
# Remove previous log handlers if they exist, to avoid duplicate logs when running multiple times
...
log_file_path = f"{base_folder}/shapes_match_{area}_gtfs{gtfs_date}_run{run_date}.osm.log"
