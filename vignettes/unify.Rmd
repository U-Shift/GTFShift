...
shape_agency <- gtfs_united$trips %>%
  ...
  distinct()
shapes_sf = GTFSwizard::get_shapes_sf(gtfs_united$shapes) %>%
  left_join(shape_agency, by = "shape_id")
mapview::mapview(shapes_sf, zcol = "agency_name", legend = TRUE)
```
</div>
<div style="flex: 1 1 0; min-width: 288px; max-width: 100%;">
### Aggregated stops
...
stop_agency <- gtfs_united$stop_times  %>%
  ...
  distinct()
stops_sf = GTFSwizard::get_stops_sf(gtfs_united$stops) %>%
  left_join(stop_agency, by = "stop_id")
mapview::mapview(stops_sf, zcol = "agency_name", legend = TRUE)
```
</div>
</div>
