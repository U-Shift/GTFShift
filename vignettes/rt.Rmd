...
To collect GTFS-RT data, use `GTFShift::rt_collect()`, which fetches data from a GTFS-RT endpoint and saves it to a CSV file for further analysis. The method runs in a loop, fetching data at regular intervals (default is every 30 seconds) until manually stopped (CTRL+C).
...
mapview::mapview(
  ...
  lane_prioritization_extended |> dplyr::filter(!is_bus_lane & frequency>=5 & !is.na(n_lanes) & n_lanes_direction>1 & avg_speed<15),
