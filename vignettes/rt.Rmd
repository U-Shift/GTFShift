...
Once GTFS-RT data is collected, it can be used to extend lane prioritization analysis. 
`GTFShift::rt_extend_prioritization()` takes a lane prioritization data frame and a GTFS-RT collection (as an `sf` object) and enriches the prioritization with real-time metrics. 
...
```{r include=FALSE}
lane_prioritization_extended = sf::st_read("https://github.com/U-Shift/GTFShift/releases/download/v0.8.1/prioritization_lisboa_rt_gtfs2026-02-04_run20260226_extended.gpkg")
```
...
The method `GTFShift::rt_commercial_speed()` implements this functionality, calculating, for each GTFS-RT update, the speed
...
For more details, refer to the method documentation. The resulting speed can then be used to extend the prioritization analysis as described above.
