...
gtfs_id = "lisboa"
gtfs = GTFShift::load_feed(data$URL[data$ID == gtfs_id])
```


# Analyse hourly frequency per stop

To analyse frequencies at stops, use `get_stop_hour_frequency()`, producing, for each, an aggregated counting of bus servicing it per hour.
...
The frequency analysis can also be performed route wise. For this purpose, use `get_route_hour_frequency()`, returning aggregated results per hour.
...
## Improve visualization with `network_overline`

Using the `overline` attribute in `get_route_frequency_hourly()` might not be the best option if the GTFS shapes do not share exactly the same geometry. In those cases, the overlapping lines might not be merged correctly, causing inconsistent results, such as a street with different frequencies along it, despite not having any bus stop in between those differences.
...
As an alternative, `GTFShift` provides the method `network_overline()`, that allows for the route aggregation. Given a simplified network, it identifies the segments corresponding to each route and uses them to aggregate the attribute defined in the parameters. It can be used to aggregate the frequencies. 
...
> GTFShift provides a method that encapsulates Python [neatnet](https://uscuni.org/neatnet/index.html) package. Refer to [vignette("donwload")](download.html#get-centerlines-for-osm-road-network) for more details.

Below is provided an example, that uses the centerlines for the Carris network as a simplification, generated using [ArcGis](https://pro.arcgis.com/en/pro-app/latest/tool-reference/cartography/merge-divided-roads.htm).

```{r}
network = sf::st_read(system.file("extdata", "centerline_carris.gpkg", 
                                  package = "GTFShift"), 
                      quiet = TRUE)
