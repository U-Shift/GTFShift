...
> In this example, we intent to get a value close to the official 757 km [reported by Carris](https://www.carris.pt/media/ikvdezmc/relat%C3%B3rio-e-contas-carris-2024.pdf){target="_blank"}. For this purpose, we use parameters `direction_wise=TRUE` to consider both directions for each route, but removing route overlaps (with `unified=TRUE`). Aditionally, we use OSM open data to get more accurate route geometries (through parameter `use_osm_routes`).
