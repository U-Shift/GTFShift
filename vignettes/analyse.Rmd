...
The frequency analysis can also be performed route wise. For this purpose, use `GTFShift::get_route_frequency_hourly`, returning aggregated results per hour.
...
As an alternative, `GTFShift` provides the method `GTFShift::network_overline`, that allows for the route aggregation. Given a target network, it identifies the segments corresponding to each route and uses them to aggregate the attribute defined in the parameters. It can be used to aggregate the frequencies. 

> One approach to build a simplified road target network could be through the centerlines determination. During the development of this project, no R packages were found suiting this purpose. [Centerline](https://github.com/atsyplenkov/centerline) package has this feature in its roadmap. Currently, there are available solutions for [Python](https://uscuni.org/neatnet/index.html) or [ArcGis](https://pro.arcgis.com/en/pro-app/latest/tool-reference/cartography/merge-divided-roads.htm).

> GTFShift provides a method that encapsulates Python [neatnet](https://uscuni.org/neatnet/index.html) package. Refer to [vignette("osm")](osm.html#get-centerlines-for-osm-road-network) for more details.
